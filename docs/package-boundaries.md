...
- `packages/graph` centralizes Microsoft Graph request behavior and may depend on infrastructure packages such as observability, but not on app surfaces.
...
- `@friendly-mail/graph`: Microsoft Graph connector, retry behavior, immutable-ID headers, and normalized mailbox DTOs
